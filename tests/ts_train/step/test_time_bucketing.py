...
def test_preprocess_time_column_not_existing(
    ...
    with pytest.raises(
        ValueError,
        match=f"Column {standard_time_bucketing.time_column_name} is not present",
    ):
...
def test_preprocess_time_column_not_timestamp(
    ...
    """
    standard_time_bucketing.time_column_name = "IMPORTO"
...
def test_buckets_monotonicity(
    ...
    all_dates = bucket_df.select(time_column_name).collect()

    for id_date in range(1, len(all_dates)):
        assert all_dates[id_date] > all_dates[id_date - 1], (
            f"during test for {time_column_name}, at row"
...
def test_bucket_df_null_values(
    ...
    contains_nulls = bucket_df.where(col(time_column_name).isNull()).count() > 0
...
def test_all_transactions_are_in_the_correct_bucket(
    ...
    final_df = final_df.withColumn(
        standard_time_bucketing.time_column_name,
        col(standard_time_bucketing.time_column_name).cast(StringType()),
    )
    final_df = final_df.withColumn(
        "bucket_start",
    ...
    all_dates = final_df.select(standard_time_bucketing.time_column_name).collect()
