...
class Aggregation(AbstractPipelineStep):
    """
    Create a new dataframe with new columns made by numeric cols using categorical cols as filters
    ...
    def _preprocess(self, df: DataFrame) -> None:
        ...
        # check if there is at least one aggregation, the dictionary must have at least variable and function (you can skip the filter)
    ...
    def _process(self, df: DataFrame, spark: SparkSession) -> DataFrame:
        ...
        # PYTEST:
        # possiamo calcolare a priori il numero di colonne che dovrebbe avere il df risultante?

        # assert len(old_df) <= len(new_df)
        # calcoalre a priori il numero di colonne che deve avere il df finale everificare che i conti tornino
        # fare un caso in cui metti solo variabili nuemriche e verichi le colonne generate
